...
export default async function handler(req, res) {
  try {
    ...
    // 🔥 STEP 2: MOBILE NUMBER SEARCH (Using new ANISHEXPLOITS API)
    if (number) {
      // Call new ANISHEXPLOITS API
      const anishexploitsUrl = `https://anishexploits.com/api/api.php?key=${ANISHEXPLOITS_KEY}&type=number&num=${encodeURIComponent(number)}`;
      console.log(`Calling ANISHEXPLOITS API: ${anishexploitsUrl}`);
      
      const anisresponse = await fetchWithTimeout(anishexploitsUrl);
      
      if (anisresponse.ok && anisresponse.data && anisresponse.data.status === "success") {
        // The API response is already in the correct format with "result" array
        const apiResults = anisresponse.data.result || [];
      ...
      // If ANISHEXPLOITS failed or no data, try backup API
    ...
    if (term && !type && !number && !aadhaarInput) {
      ...
      if (detectedType === 'mobile') {
        const anishexploitsUrl = `https://anishexploits.com/api/api.php?key=${ANISHEXPLOITS_KEY}&type=number&num=${encodeURIComponent(term)}`;
        const anisresponse = await fetchWithTimeout(anishexploitsUrl);
