...
export default async function handler(req, res) {
  try {
    ...
    // ✅ API KEYS (Hidden - response mein nahi dikhengi)
    ...
    const resultData = {
      ...
      // other_types: []  // 👈 Completely removed from response
    };
    
    // Internal storage for other types data (ye response mein nahi jayega)
    ...
    // Detected type for internal other types
    const detectedType = type || detectInputType(term || number || aadhaarInput);

    // 🔄 STEP 1: AGAR KOI SPECIFIC TYPE DIYA HAI (internal other types ke liye)
    if (type && !['mobile', 'id_number', 'id_family'].includes(type)) {
      ...
      if (rOtherType.ok && rOtherType.data) {
        // Internally store but not in response
        ...
        // Optional: Agar chahein to kuch specific other types ka data 
        // main response mein bhi daal sakte hain. Example:
        // if (type === 'pan' || type === 'vehicle_num') {
        //   resultData.some_field = rOtherType.data;
        // }
      }
    }
    ...
    const anyGood =
      ...
      internalOtherTypes.length > 0;  // internal use ke liye check
    ...
    return res.status(200).json({
      ...
      message: anyGood
        ...
        : "कोई डेटा नहीं मिला।",
      summary: summary,
      data: resultData,  // other_types हटा दिया गया है
      supported_types: [
      ...
      ]  // 👈 owner/developer हटा दिया गया है
    });
