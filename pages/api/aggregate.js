...
export default async function handler(req, res) {
  try {
    ...
    const SPLEXXO_KEY = process.env.SPLEXXO_KEY || "SPLEXXO222_3"; // Updated default key
    ...
    // 🔄 STEP 1: New Splexxo API for number information (Updated API)
    if (number && !aadhaarInput) {
      ...
      if (rNumberInfo.ok && rNumberInfo.data && 
          ...
          rNumberInfo.data.data.success) {
        
        const apiData = rNumberInfo.data.data;
        
        // New API response format according to given example
