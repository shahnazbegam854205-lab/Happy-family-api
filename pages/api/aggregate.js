...
export default async function handler(req, res) {
  try {
    ...
    // ✅ DIRECT API KEYS (NO ENV)
    const ALLAPI_KEY = "DEMOKEY";
    const RATION_KEY = "paidchx";
    const SPLEXXO_KEY = "SPLEXXO222_3"; // ✅ Direct key here
    ...
    // 🔄 STEP 1: NEW SPLEXXO API
    if (number && !aadhaarInput) {
      // ✅ Direct API call with hardcoded key
      const numberInfoUrl = `https://splexxo-api.vercel.app/api?number=${encodeURIComponent(number)}&key=${SPLEXXO_KEY}`;
      ...
      if (rNumberInfo.ok && rNumberInfo.data) {
        ...
        // Format 1: New format with nested data
        if (rNumberInfo.data.status === "success" && 
            rNumberInfo.data.data && 
            rNumberInfo.data.data.success === true) {
          apiData = rNumberInfo.data.data;
        }
        // Format 2: Direct success with result
        else if (rNumberInfo.data.success === true) {
          apiData = rNumberInfo.data;
        }
        // Format 3: Maybe data is directly in result
        else if (rNumberInfo.data.result) {
          apiData = rNumberInfo.data;
        }
    ...
    if (idsToProcess.length > 0) {
      for (const idVal of idsToProcess) {
        const rationUrl = `https://happy-ration-info.vercel.app/fetch?key=${RATION_KEY}&aadhaar=${encodeURIComponent(idVal)}`;
        const allApiUrl = `https://allapiinone.vercel.app/?key=${ALLAPI_KEY}&type=id_number&term=${encodeURIComponent(idVal)}`;
