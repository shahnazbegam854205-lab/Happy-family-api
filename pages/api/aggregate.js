...
export default async function handler(req, res) {
  try {
    ...
    // ✅ API KEYS (ye response mein nahi dikhengi)
    ...
    const resultData = {
      ...
      // ❌ other_types hata diya - response mein nahi dikhega
    };
    ...
    if (type && !['mobile', 'id_number', 'id_family'].includes(type)) {
      ...
      if (rOtherType.ok && rOtherType.data) {
        ...
        // 🔥 Agar chahein to specific other types ka data 
        // main response mein bhi daal sakte hain. Example:
        // if (type === 'pan') {
        //   resultData.pan_info = rOtherType.data;
        // }
      }
    }
      ...
      for (const idVal of allIdsToProcess) {
        ...
        if (aadharResponse.ok && aadharResponse.data?.success) {
          resultData.aadhar.push({
            id: idVal,
            data: aadharResponse.data  // name fields ke saath
          });
        ...
        if (rationResponse.ok && rationResponse.data?.success) {
          resultData.ration.push({
            id: idVal,
            data: rationResponse.data  // name fields ke saath
          });
        }
      }
    }
      ...
      for (const mobileNum of uniqStrings(allMobilesToProcess)) {
        ...
        if (!rMobileInfo.ok || !rMobileInfo.data?.data?.length) {
          ...
          if (rMobileInfo.ok && rMobileInfo.data?.success && rMobileInfo.data?.result?.results) {
            // Mobile info store karo (name fields ke saath)
    ...
    const anyGood =
      ...
      internalOtherTypes.length > 0;  // internal use ke liye
